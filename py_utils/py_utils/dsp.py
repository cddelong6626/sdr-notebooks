...
def rrc(n_taps=21, beta=0.35, Ts=2):
    ...
    # initialize vectors
    h = np.empty(n_taps, dtype=np.complex64)
...
def rrc_filter(signal, n_taps=21, beta=0.35, Ts=2, trim=True):
    taps = rrc(n_taps, beta, Ts)
    y = np.convolve(signal, taps)
    if trim:
        trim_amt = (n_taps - 1) // 2
        return y[trim_amt: -trim_amt] if trim_amt > 0 else y
    else:
        return y
