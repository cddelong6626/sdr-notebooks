...
TEST(PIDFeedbackTest, ZeroGainReturnsZero) {
    ...
    EXPECT_FLOAT_EQ(pid.update(1.0f), 0.0f);
    EXPECT_FLOAT_EQ(pid.update(-1.0f), 0.0f);
}

TEST(PIDFeedbackTest, ProportionalOnly) {
    PIDFeedback pid(2.0f, 0.0f, 0.0f);
    EXPECT_FLOAT_EQ(pid.update(1.5f), 3.0f);
    EXPECT_FLOAT_EQ(pid.update(-2.0f), -4.0f);
}

TEST(PIDFeedbackTest, IntegralOnly) {
    PIDFeedback pid(0.0f, 1.0f, 0.0f);
    EXPECT_FLOAT_EQ(pid.update(1.0f), 1.0f);
    EXPECT_FLOAT_EQ(pid.update(2.0f), 3.0f);  // sum_e = 1+2=3
    EXPECT_FLOAT_EQ(pid.update(-1.0f), 2.0f); // sum_e = 3+(-1)=2
}

TEST(PIDFeedbackTest, DerivativeOnly) {
    PIDFeedback pid(0.0f, 0.0f, 1.0f);
    EXPECT_FLOAT_EQ(pid.update(1.0f), 1.0f);  // prev_e=0, e=1, diff=1
    EXPECT_FLOAT_EQ(pid.update(4.0f), 3.0f);  // prev_e=1, e=4, diff=3
    EXPECT_FLOAT_EQ(pid.update(2.0f), -2.0f); // prev_e=4, e=2, diff=-2
}

TEST(PIDFeedbackTest, PIDCombined) {
    PIDFeedback pid(1.0f, 0.5f, 0.1f);
    float out1 = pid.update(2.0f);                                           // sum_e=2, prev_e=2
    EXPECT_FLOAT_EQ(out1, 1.0f * 2.0f + 0.5f * 2.0f + 0.1f * 2.0f);          // 2+1+0.2=3.2
    float out2 = pid.update(3.0f);                                           // sum_e=5, prev_e=3
...
TEST(PIDFeedbackTest, MultipleUpdates) {
    PIDFeedback pid(0.5f, 0.2f, 0.1f);
    float out1 = pid.update(1.0f);                                             // sum_e=1, prev_e=1
    float out2 = pid.update(2.0f);                                             // sum_e=3, prev_e=2
    float out3 = pid.update(-1.0f);                                            // sum_e=2, prev_e=-1
